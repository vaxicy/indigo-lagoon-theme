...
.tea.small p{font-size:13px;margin-top:18px;color:#F2F2ED}
.tea.small:after{content:'';position:absolute;left:0;right:0;bottom:0;height:14px;background:#7CD5C7}
.tea.wide{width:1400px;height:560px;background:#7CD5C7;color:#464B71}
...
.tea .window{position:absolute;left:550px;top:83px;transform:scale(.62);transform-origin:top left;border:2px solid #118AB2;border-radius:14px;overflow:hidden;color:var(--ink)}
...
css += """
.poster{width:1400px;height:560px;background:var(--milk);position:relative;overflow:hidden;text-align:center;border-top:8px solid #118AB2}
...
"""
small='<div class="tea small"><img src="'+logo+'"><h1>Indigo Lagoon</h1><div class="sub">CHROME THEME</div><p>A refreshing space to browse.</p></div>'
wide='<div class="poster"><h1>Indigo Lagoon Theme</h1><p>Muted indigo. Fresh mint. A little coastal calm.</p><div class="preview">'+browser(True)+'</div></div>'
colors=[('Muted Indigo','frame','Window frame','#F2F2ED'),('Lagoon Blue','ntp_link','Links & accents','#FFFFFF'),('Fresh Mint','toolbar','Toolbar & active tab','#464B71'),('Mist White','ntp_background','New tab background','#464B71')]
...
intro='<div class="intro"><div class="kicker">A LITTLE COASTAL CALM</div><h1>Indigo Lagoon Theme</h1><p>Four colors. One refreshing space.</p><div class="cards">'+cards+'</div><p style="font-size:16px;margin-top:30px">Solid colors · Minimal design · No wallpaper · No permissions required</p></div>'
